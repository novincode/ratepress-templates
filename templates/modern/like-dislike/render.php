...
<div class="ratepress-widget ratepress-likedislike<?php echo $is_js_mode ? ' ratepress-js-mode' : ''; ?>"<?php echo $theme === 'dark' ? ' data-theme="dark"' : ''; ?>
     ...
     role="group"
     aria-label="<?php esc_attr_e('Like or dislike rating widget', 'ratepress'); ?>">
    ...
    <div class="likedislike-buttons">
        <button class="like-btn <?php echo $user_liked ? 'active' : ''; ?>"
                ...
                aria-label="<?php echo esc_attr($user_liked ? __('Remove like', 'ratepress') : __('Like this', 'ratepress')); ?>"
                aria-describedby="like-count-<?php echo esc_attr($object_id); ?>"
                title="<?php echo esc_attr($user_liked ? __('Remove like', 'ratepress') : __('Like this', 'ratepress')); ?>">
            ...
            <?php if ($show_counts): ?>
                <span class="count" 
                      ...
                      aria-label="<?php esc_attr_e('Number of likes:', 'ratepress'); ?> <?php echo esc_attr($likes); ?>">
        ...
        <button class="dislike-btn <?php echo $user_disliked ? 'active' : ''; ?>"
                ...
                aria-label="<?php echo esc_attr($user_disliked ? __('Remove dislike', 'ratepress') : __('Dislike this', 'ratepress')); ?>"
                aria-describedby="dislike-count-<?php echo esc_attr($object_id); ?>"
                title="<?php echo esc_attr($user_disliked ? __('Remove dislike', 'ratepress') : __('Dislike this', 'ratepress')); ?>">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            ...
            <?php if ($show_counts): ?>
                <span class="count" 
                      ...
                      aria-label="<?php esc_attr_e('Number of dislikes:', 'ratepress'); ?> <?php echo esc_attr($dislikes); ?>">
    ...
    <?php if ($show_percentage && $total > 0): ?>
        <div class="likedislike-percentage">
            <?php echo esc_html($percentage); ?>% <?php esc_e('positive', 'ratepress'); ?>
        </div>
    <?php endif; ?>
</div>
